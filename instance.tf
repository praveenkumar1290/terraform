resource "aws_instance" "instance" {
  ...
  provisioner "remote-exec" {
    inline = ["sudo apt-get update","sudo apt install python3 -y",
    "sudo apt-get install ansible -y","sudo apt-get update -y",
...
resource "aws_key_pair" "deployer" {
  key_name   = "aws_key"
  public_key = "use your public key which you generator"
}